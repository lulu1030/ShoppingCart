                            ...
                            <div class="address-form">
                                <div class="module-form-row">
                                    <div class="form-item-v3">
                                        <i>收货人姓名</i>
                                        <input type="text" class="js-verify" v-model="recieve.name">
                                ...
                                <div class="module-form-row">
                                    <div class="form-item-v3">
                                        <i>手机号</i>
                                        <input type="text" class="js-verify" v-model="recieve.phone">
                                        <div class="verify-error"></div>
                                    </div>
                                </div>
                                <div class="module-form-row clear">
                                    <div class="form-item-v3 area-code-w fn-left form-valid-item">
                                        <i>区号（可选）</i>
                                        <input type="text" class="js-verify js-address-area-code" v-model="recieve.areaCode">
                                        <div class="verify-error"></div>
                                    </div>
                                    <div class="form-item-v3 telephone-w fn-right form-valid-item">
                                        <i>固定电话（可选）</i>
                                        <input type="text" class="js-verify js-address-telephone" v-model="recieve.landLine">
                                        <div class="verify-error"></div>
                                    </div>
                                </div>
                                <div class="module-form-row clear">
                                    <div class="form-item-v3 select-item province-wrapper">
                                        <select name="province_code" class="province select-province js-form-province js-verify" v-model="recieve.provinceId">
                                            <option value="0">请选择省份</option>
                                            <option :value="provice.area_id" v-for="provice,index in addList">{{provice.area_name}}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="module-form-row clear">
                                    <div class="form-item-v3 select-item city-wrapper fn-left form-focus-item">
                                        <select class="city select-city js-form-city js-verify" v-model="recieve.cityId">
                                    ...
                                    <div class="form-item-v3 select-item district-wrapper fn-right form-focus-item">
                                        <select class="city select-city js-form-city js-verify" v-model="recieve.countyId">
                                ...
                                <div class="module-form-row">
                                    <div class="form-item-v3">
                                        ...
                                        <input type="text" class="js-verify" v-model="recieve.add">
                                        <div class="verify-error"></div>
                                    </div>
                                </div>
                                <div class="module-form-row fn-clear">
                                    <input type="checkbox" class="hide" v-model="recieve.default">
                                    <span class="blue-checkbox" :class="{'blue-checkbox-on':recieve.default}" @click="checkDefault"></span>设为默认
                                </div>
                                <div class="dialog-blue-btn big-main-btn js-verify-address" 
                                	:class="{' disabled-btn':!isAll}">
                                    <a>保存</a>
                                </div>
                            </div>
                        </div>
...
<script>
	...
	export default {
		...
		watch:{
			'recieve.county' () {
				this.countyList.forEach((city)=>{
					if(county.area_id === this.recieve.countyId){
						this.recieve.county = county.area_name
					}
				})
			},
			recieve () {
		...
		computed:{
			cityList () {
				let cityList = []
				this.addList.forEach((provice)=>{
					if(provice.area_id === this.recieve.provinceId){
						cityList = provice.city_list
						this.recieve.province = provice.area_name
					}
				})
				if(this.recieve.provinceId == 0){
					this.recieve.cityId = 0
					this.recieve.countyId = 0
				}
				return cityList
			...
			countyList () {
				...
				this.cityList.forEach((city)=>{
					if(city.area_id === this.recieve.cityId){
						countyList = city.county_list
						this.recieve.city = city.area_name
					}
				})
				if ( this.recieve.cityId == 0 ) {
					this.recieve.countyId = 0
				}
				return countyList
		...
		methods:{
			...
			checkDefault () {
				this.recieve.default = !this.recieve.default
			},
			buttonSure () {
				if(this.recieve.name&&this.recieve.phone&&this.recieve.county&&this.recieve.city&&this.recieve.add&&this.recieve.province){
					isAll = true
				}
			}
		}
