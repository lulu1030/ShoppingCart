			...
			<div class="store-content">
				<div class="cart-box">
					...
					<div class="cart-inner">
						<div class="empty-label" v-if="countTotle<=0">
							<h3>您的购物车中还没有商品</h3>
						...
						<div v-else>
							...
							<div class="cart-table">
								<div class="cart-group">
									<!--购物列表-->
									<div class="cart-top-items" v-for='item,index in carList'>
										<div class="cart-items">
											<div class="items-choose">
												<span class="blue-checkbox-new" :class="{'checkbox-on':item.checked}" @click="checkedCar(item.sku_id)">
													<a></a>
												</span>
											</div>
											...
											<div class="operation">
												<a class="items-delete-btn"  @click="delCard(item.sku_id)"></a>
											</div>
											<div class="subtotal">¥ {{item.price*item.count}}</div>
											<div class="item-cols-num">
												<div class="select js-select-quantity">
													<span class="down" @click="subCarNum(item.sku_id)">-</span>
													<span class="num">
														{{item.count}}
													</span>
													<span class="up" @click="addCarNum(item.sku_id)">+</span>
					...
					<div class="cart-bottom-bg fix-bottom" v-if="countTotle>0">
						<div class="cart-bar-operation">
							<div>
								<div class="choose-all js-choose-all">
									<span class="blue-checkbox-new" :class="{'checkbox-on':allChecked}" @click="allCheckedCar(allChecked)"><a></a></span>
									全选
								</div>
...
<script>
	export default {
		...
		methods:{
			delCard (id) {
				this.$store.commit('deleCardPanel',id)
			},
			addCarNum (id) {
				this.$store.commit('addCarNum',id)
			},
			subCarNum (id) {
				this.$store.commit('subCarNum',id)
			},
			checkedCar (id) {
				this.$store.commit('checkedCar',id)
			},
			allCheckedCar (allChecked) {
				this.$store.commit('allCheckedCar',allChecked)
			},
			delChecked () {
